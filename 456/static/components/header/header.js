...
(function($,window,document,undefined){
	...
	cotrPlugin.prototype = {
		...
		appendElement:function(){
			...
			$('.main-header').css('margin-left',this.settings.margin);
			if(!this.settings.showMisHome){
				$('.right-nav li:first-child').css('display','none')
			}
			if(!this.settings.showUser){
				$('.right-nav').css('display','none');
				$('.user').css('display','none')
			}
			console.log($('.main-header').css('margin-left'))
			this.render();
		},
		// 渲染数据
		render:function(){
